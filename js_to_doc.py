...
def parse(js_file, classes):
    with open(js_file, 'r', encoding='gbk', errors='ignore') as f:
        lines = f.readlines()
        class_dedine_line = [line for line in lines if '= function' in line and '.prototype' not in line][0]
        class_name = class_dedine_line.split('=')[0].strip()
        pos = lines.index(class_dedine_line)
        description = lines[pos - 1].replace('//', '').strip().split()[1]

        method_define_lines = [line for line in lines if '.prototype' in line and not line.startswith('//')]

        method_list = []
        for e in method_define_lines:
        	pos = lines.index(e)
        	comment = lines[pos-1].strip().split()[1] if len(lines[pos-1].split())==2 else ''
        	prototype = e.split('=')[0]
        	method_name = prototype.split('.')[-1]
        	return_type = 'var'
        	l_brace = e.find('(')
        	r_brace = e.find(')')
        	parameter = e[l_brace + 1: r_brace]
        	access = 'public'
...
def get_arguments():
    ...
    parser.add_argument('src_dir', help='A js source folder')
...
def main():
    # 解析命令行参数
    args = get_arguments()
    js_files = list_js_files(args.src_dir)

    # 解析js文件
    classes = []
    for js_file in js_files:
        parse(js_file, classes)
